...
public extension WeightUnit {
    /// 获取单位的本地化名称（默认跟随系统语言，当前支持中文/英文）。
    ///
    /// - Parameter locale: 指定语言环境，默认使用 `Locale.current`。
    /// - Returns: 本地化后的单位名称。
    func localizedName(locale: Locale = .current) -> String {
        let isChinese = locale.identifier.hasPrefix("zh") || locale.language.languageCode?.identifier == "zh"
        switch self {
